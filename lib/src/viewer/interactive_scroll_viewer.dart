...
export 'package:pdfx_plus/src/viewer/pdf_page_image_provider.dart';
export 'package:photo_view/photo_view.dart';
export 'package:photo_view/photo_view_gallery.dart';

/// Triggers when double tap zoom invoked.
typedef _DoubleTapZoomInvokedCallback = Offset Function(Offset value, Offset tapPosition);
...
class InteractiveScrollViewerState extends State<InteractiveScrollViewer> {
  ...
  void scrollTo(Offset offset) {
    if (widget.transformationController != null) {
      final Offset previousOffset = widget.transformationController!.toScene(Offset.zero);
  ...
  void _handleDoubleTap() {
    if (widget.transformationController != null) {
      ...
      const double maximumZoomLevel = 3;
      final double zoomLevel = widget.transformationController!.value.getMaxScaleOnAxis();
      Offset normalizedOffset;
      Offset offset;
      if (zoomLevel <= minimumZoomLevel) {
        normalizedOffset =
            (-_tapPosition - widget.transformationController!.toScene(Offset.zero) * zoomLevel) / zoomLevel;
        scaleTo(maximumZoomLevel);
        offset = (-_tapPosition - normalizedOffset * maximumZoomLevel) / maximumZoomLevel;
        ...
        offset = (-_tapPosition - normalizedOffset * minimumZoomLevel) / minimumZoomLevel;
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onDoubleTapDown: widget.enableDoubleTapZooming ? _handleDoubleTapDown : null,
